...
export interface INetworkConnection {
    addOnConnectionCallback(onConnectionCallback: (identifier: string) => Promise<boolean>): void;
    addOnCloseCallback(onConnectionCallback: (identifier: string) => Promise<boolean>): void;
    addOnMessageCallback(onMessageCallback: (identifier: string, message: string) => Promise<boolean>): void;
...
export class WebsocketNetworkConnection implements INetworkConnection {
    ...
    private _onConnectionCallbacks: ((identifier: string) => Promise<boolean>)[] = [];
    private _onCloseCallbacks: ((identifier: string) => Promise<boolean>)[] = [];
    private _onMessageCallbacks: ((identifier: string, message: string) => Promise<boolean>)[] = [];
    ...
    constructor(
        ...
        this._configs.forEach(websocketServerConfig => {
            ...
            _httpServer.on('upgrade', (request, socket, head) =>
                this._upgradeRequest(request, socket, head, _socketServer, websocketServerConfig.securityProfile));
    ...
    addOnCloseCallback(onCloseCallback: (identifier: string) => Promise<boolean>): void {
    ...
    private async _upgradeRequest(req: http.IncomingMessage, socket: Duplex, head: Buffer, wss: WebSocketServer, securityProfile: number) {
        ...
        // Register client
        const registered = await this._cache.set(clientConnection.identifier, JSON.stringify(instanceToPlain(clientConnection)), CacheNamespace.Connections);
        if (!registered) {
